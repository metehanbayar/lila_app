...
function StoreCard({ restaurant, onClick, className = '' }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group w-full overflow-hidden rounded-[28px] border border-white/70 bg-white text-left shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card-hover ${className}`.trim()}
    >
